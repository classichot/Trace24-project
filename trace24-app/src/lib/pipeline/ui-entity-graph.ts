...
export function buildUiEntityGraph(input: {
  ...
  for (const m of relatedMatches.slice(0, 6)) {
    ...
    details[pid] = {
      ...
      sub:
        m.matchType === 'surname'
          ? 'นามสกุลร่วม · lead ไม่ใช่ข้อพิสูจน์'
          : m.matchType === 'full_name'
            ? 'ชื่อเต็มตรงกัน · รอยืนยัน'
            : `${m.matchType || 'ความสัมพันธ์'} · รอยืนยัน`,
      facts: [
        m.explanation || 'พบความเชื่อมโยงจากนามสกุล/ทำเนียบ/กรรมการ',
        m.matchType === 'surname'
          ? 'นามสกุลร่วมเป็น lead ให้สอบสวนเท่านั้น — ไม่ใช่ข้อพิสูจน์เครือญาติหรือการทุจริต'
          : 'ต้องยืนยันด้วยเอกสารต้นทางก่อนสรุป',
      ].filter(Boolean),
      docs: [],
      link: null,
    };
  ...
  return {
    ...
    meta: {
      ...
      graphNote: edgeCount
        ? relatedMatches.length
          ? `แสดง ${edgeCount} ความสัมพันธ์ · R5/R13 จากทำเนียบ/กรรมการเป็น lead (นามสกุลร่วม ≠ ข้อพิสูจน์)`
